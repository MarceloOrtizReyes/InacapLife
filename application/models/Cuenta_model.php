...
class Cuenta_model extends CI_Model{
	...
	public function __construct(){
        ...
        public function findById($id = null) {
                $id = intval($id);
                $this->load->database();
                $res    = $this->db->get_where(self::$_table, array('cue_id' => $id));
                $result = null;
                if ($res->num_rows() == 1) {
        ...
        public function save() {
                try {
                        $this->load->database();
                        if ($this->_columns['cue_id'] == 0 || is_null($this->_columns['cue_id'])) {
                                $this->db->insert(self::$_table, $this->_columns);
                                $this->_columns['cue_id'] = $this->db->insert_id();
                        } else {
                                $this->db->where('cue_id', $this->_columns['cue_id']);
