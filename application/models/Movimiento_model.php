...
public $_columns = array(
	...
	private static $_table = 'Movimiento';
	public function __construct(){
        ...
        public function findById($id = null) {
                $id = intval($id);
                $this->load->database();
                $res    = $this->db->get_where(self::$_table, array('mov_id' => $id));
                $result = null;
                if ($res->num_rows() == 1) {
        ...
        public function save() {
                try {
                        $this->load->database();
                        if ($this->_columns['mov_id'] == 0 || is_null($this->_columns['mov_id'])) {
                                $this->db->insert(self::$_table, $this->_columns);
                                $this->_columns['mov_id'] = $this->db->insert_id();
                        } else {
                                $this->db->where('mov_id', $this->_columns['mov_id']);
